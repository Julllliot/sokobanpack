...
# @context any
# @output
#   spawns 4 markers with different ids at the player's feet and kills it next tick

tag @a[limit=1] add this

execute positioned as @a[limit=1,tag=this] run summon marker ~ ~ ~ {Tags:["id_sorter","id0","sokoban.entity","temp"]}
execute positioned as @a[limit=1,tag=this] run summon marker ~ ~ ~ {Tags:["id_sorter","id1","sokoban.entity","temp"]}
execute positioned as @a[limit=1,tag=this] run summon marker ~ ~ ~ {Tags:["id_sorter","id2","sokoban.entity","temp"]}
execute positioned as @a[limit=1,tag=this] run summon marker ~ ~ ~ {Tags:["id_sorter","id3","sokoban.entity","temp"]}
...
schedule function sokoban:util/clear_temp 1t
